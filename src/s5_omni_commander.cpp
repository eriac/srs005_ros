...
void calculation(float *out, float *in){
  for(int i=0;i<3;i++){
    out[i]=(cos(wheel_normal[i])*in[0]+sin(wheel_normal[i])*in[1])/wheel_radius*wheel_ratio;
    out[i]+=-in[2]*wheel_base/wheel_radius*wheel_ratio;
  }
}
...
int main(int argc, char **argv)
{
  ...
  //publish
  ros::Publisher wheel0_pub = n.advertise<std_msgs::Float32>("wheel0", 10);
  ros::Publisher wheel1_pub = n.advertise<std_msgs::Float32>("wheel1", 10);
  ros::Publisher wheel2_pub = n.advertise<std_msgs::Float32>("wheel2", 10);
  //Subscribe
